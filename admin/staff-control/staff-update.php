...
if(isset($_POST["update"])){
    ...
    $query ="UPDATE `admin` SET `username`='$name',`email`='$email',`Role`='$role'WHERE id = $id";
            ...
            $query ="UPDATE `admin` SET `username`='$name',`email`='$email',`password`='$pass'WHERE id = $id";
        }
    $result = mysqli_query($con, $query);
    if($result){
        echo "<script>alert('Staff Updated');window.location='staff-account.php'</script>";
    }else{  

        echo "<script>alert('Not updated')</script>";
    }
}else{
    $Error = "<div class='alert alert-danger'>Do not leave empty fields!</div>";
}
}
?>
<form method="post">
    ...
    <h3>Account Update Details:</h3>
    <?=$Error ?? null?>
    <form role="form">
        <div class="form-group input-group">
            <span class="input-group-addon"><i class="fa fa-user"></i></span>
            <input type="text" class="form-control" placeholder="Username" value="<?=$ename?>" name="name">
        </div>        
        <div class="form-group input-group">
            <span class="input-group-addon"><i class="fa fa-envelope"></i></span>
            <input type="text" class="form-control" placeholder="Email" value="<?=$eemail?>" name="email">
        </div>     
    <?php 
    if($_SESSION["admin"] == 'Staff'){
   echo 
        '<div class="form-group input-group">
           ...
           echo '<div class="form-group">
                     <label>Role:</label>
                        <select class="form-control" name="role">
                          ...
                          <option '.$reception.'>Receptionist</option>
                        </select>
                 </div>';
         }
         ?>
    <button type="submit" name="update" class="btn btn-danger">Update</button>
    <a href="staff-account.php" class="btn btn-danger d-flex justify-content-between">Back</a>
</form>
